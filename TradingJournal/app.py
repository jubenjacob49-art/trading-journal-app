...
DB_PATH = BASE_DIR / "trading_journal.db"
TRADE_IMAGE_DIR = BASE_DIR / "trade_images"
LOGO_CANDIDATES = [
    BASE_DIR / "logo.png",
    BASE_DIR / "logo.jpg",
    BASE_DIR / "logo.jpeg",
...
def update_trade(
    ...
    if new_image_path.strip():
        ...
        if old_image_path and old_image_path != final_image_path:
            old_file = resolve_image_path(old_image_path)
            if old_file.exists():
...
def delete_trade(conn: sqlite3.Connection, trade_id: int, user_id: int) -> bool:
    ...
    if row and row["image_path"]:
        image_file = resolve_image_path(str(row["image_path"]))
    ...
    with tab2:
        ...
        if trades_df.empty:
            ...
            if not image_trades.empty:
                ...
                resolved_preview_image = resolve_image_path(str(selected_image_path))
                if selected_image_path and resolved_preview_image.exists():
                    img_col_l, img_col_m, img_col_r = st.columns([1, 2, 1])
                    with img_col_m:
                        st.image(str(resolved_preview_image), use_container_width=True)
                else:
            ...
            if not trades_df.empty:
                ...
                resolved_current_image = resolve_image_path(current_image_path)
                if current_image_path and resolved_current_image.exists():
                    st.caption("Current image")
                    st.image(str(resolved_current_image), width=180)
